...
class _NewPlaceState extends State<NewPlace> {
  ...
  void saveNewPlace() async {
    try {
      String nuevoLugar = _tipoController.text;
      String horasStr =
      ...
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        List<dynamic> tipos = [];
        if (lugaresSnapshot.exists && lugaresSnapshot.data() != null) {
          var data = lugaresSnapshot.data() as Map<String, dynamic>;
          if (data.containsKey('tipos')) tipos = List.from(data['tipos']);
        }

        List<dynamic> pruebaHoras = [];
        if (horasSnapshot.exists && horasSnapshot.data() != null) {
          var data = horasSnapshot.data() as Map<String, dynamic>;
          if (data.containsKey('prueba_horas'))
            pruebaHoras = List.from(data['prueba_horas']);
        }

        if (!tipos.contains(nuevoLugar)) {
          tipos.add(nuevoLugar);
          // Pad the hours array if it's shorter than the types array before adding the new element
          while (pruebaHoras.length < tipos.length - 1) {
            pruebaHoras.add("44");
          }
          pruebaHoras.add(horasStr);

          transaction.update(lugaresRef, {'tipos': tipos});
          transaction.set(
              horasRef, {'prueba_horas': pruebaHoras}, SetOptions(merge: true));
        }
      });
